...
class IbanBlocklistTest extends TestCase {
	...
	public function testGivenEmptyBlocklist_ibanIsNotBlocked() {
		$blocklist = new IbanBlocklist( [] );

		$this->assertFalse( $blocklist->isIbanBlocked( $this->iban ) );
	}

	public function testGivenNonmatchingBlocklist_ibanIsNotBlocked() {
		$blocklist = new IbanBlocklist( [ 'DE56500700100000020123' ] );

		$this->assertFalse( $blocklist->isIbanBlocked( $this->iban ) );
	}

	public function testGivenMacthingBlocklist_ibanIsBlocked() {
